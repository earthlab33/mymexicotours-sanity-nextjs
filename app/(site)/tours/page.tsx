...
export default function Tours() {
    ...
    const pastTours = tours.filter(tour => !tour.upcoming);

    const renderTours = (toursList: Tour[]) => (
        toursList.map((tour) => (
    ...
    return (
        <div className="max-w-6xl pt-10 mx-auto">
            ...
            <h2 className="text-slate-100 text-xl font-bold text-center mt-6 pb-6">Upcoming Tours</h2>
            <div className="grid overflow-hidden sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-3 gap-5 grid-flow-row w-auto 2xl:h-auto 2xl:square">
                {renderTours(upcomingTours)}
            </div>
            
            <h2 className="text-slate-100 text-xl font-bold text-center mt-12 pb-6">Past Tours</h2>
            <div className="grid overflow-hidden sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-3 gap-5 grid-flow-row w-auto 2xl:h-auto 2xl:square">
                {renderTours(pastTours)}
            </div>
        </div>
    );
}
